...
export const intakeSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        fetchIntake.fulfilled,
        (state: IntakeState, action: PayloadAction<IntakeEntity[]>) => {
...
export const getIntakeState = (rootState: unknown): IntakeState =>
  rootState[INTAKE_FEATURE_KEY];

export const selectAllIntake = createSelector(getIntakeState, selectAll);

export const selectIntakeEntities = createSelector(
  getIntakeState,
  selectEntities
);
